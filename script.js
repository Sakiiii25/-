...
const remakeBtn = document.getElementById('remake-btn'); // 追加
...
// 別の文章を作り直すボタンを押した時の動き
remakeBtn.addEventListener('click', () => {
  // 今表示されている状態（文章・画像）を履歴に保存する
  saveToHistory();

  // もう一度、生成ボタンと同じ処理を呼び出す（入力欄や画像はそのまま維持されています）
  // loader等の初期化処理を直接呼ぶ
  startLoading();
  // ページの上に戻る
  window.scrollTo({ top: 0, behavior: 'smooth' });
...
function saveToHistory() {
  const currentText = generatedTextArea.textContent;
  if (!currentText || currentText === "ここに文章が出ます") return; // まだ作られていない時は保存しない
  ...
  // もし写真があれば、容量節約のために小さな画像（サムネイル）を作って保存する
  if (currentImage && !editorContainer.classList.contains('hidden')) {
    ...
    // 現在のフィルターを適用
    if (currentFilter === 'slr') {
      tCtx.filter = 'contrast(1.1) saturate(1.2)';
    } else if (currentFilter === 'purikura') {
      tCtx.filter = 'brightness(1.2) contrast(0.9) saturate(1.3) sepia(0.2) hue-rotate(-10deg)';
    }
    
    // 画像を縮小して書き込む
    tCtx.drawImage(imageCanvas, 0, 0, thumbCanvas.width, thumbCanvas.height);
    
    // 画質を落として軽いデータにする（0.5）
    thumbnailData = thumbCanvas.toDataURL('image/jpeg', 0.5);
  }

  // 保存するデータのまとまり（1件分）
  const historyItem = {
    id: Date.now(), // 1件ずつ被らない背番号
    text: currentText,
    image: thumbnailData,
    date: dateStr
  };
  ...
  // 以前の履歴を読み込む
  let history = JSON.parse(localStorage.getItem('snsMakerHistory') || '[]');
  
  // 同じID（または直前と全く同じ内容）が連続しないかチェック
  if (history.length > 0) {
    const lastItem = history[0];
    if (lastItem.text === historyItem.text && lastItem.image === historyItem.image) {
      return; // 全く同じものが続いたら保存しない（二重保存防止）
    }
  }
  ...
  // 記憶スペース（ローカルストレージ）に書き込む
  localStorage.setItem('snsMakerHistory', JSON.stringify(history));

  // 画面の履歴一覧も更新する
  renderHistory();
}

// 記憶スペースから履歴を読み込んで画面に表示する関数
function renderHistory() {
  const history = JSON.parse(localStorage.getItem('snsMakerHistory') || '[]');
  ...
  history.forEach(item => {
    ...
    const imgHtml = item.image 
      ? `<img src="${item.image}" alt="履歴の写真" class="history-image">`
      : `<div class="history-image" style="display:flex; align-items:center; justify-content:center; color:#ccc; font-size:10px;">No Image</div>`;
    ...
    // カードをクリックしたら、その結果を画面に復元する（おまけ機能）
    card.addEventListener('click', () => {
      generatedTextArea.textContent = item.text;
      
      // 画像はサムネイルから復元するのは画質が粗いため、文章だけの復元とします
      // （画像を完全に復元するには元の重いデータを保存する必要があるため）
      inputSection.classList.add('hidden');
      resultSection.classList.remove('hidden');
      ...
      currentImage = null;
      editorContainer.classList.add('hidden');
      resultUploadContainer.classList.remove('hidden');
...
clearHistoryBtn.addEventListener('click', () => {
  if (confirm('履歴をすべて消去しますか？（元には戻せません）')) {
    localStorage.removeItem('snsMakerHistory');
    renderHistory();
  }
});
